...
The coordinates are in **WGS84** format (longitude and latitude), which are converted to local distances in meters using an **equirectangular approximation**, which is accurate enough for the small distances in this work. Long and lat do not correspond to equal distances, so it is not possible to directly rasterize the data without this conversion. The circulatirty must be evaluated projecting the ring road according to the local normal projection, approximated by the local distances.
...
Next, the rasterized image is **dilated 32.5 m** (kernel size 13) to ensure that the road is continuous, as every road has an initial thickness of 1 pixel. 
...
The mask is finally **eroded 22.5 m** (kernel size 9) to give some width to the road, as the initial rasterized image is only one pixel or 5 m wide. The final road width is hence at least **20 m**.  
