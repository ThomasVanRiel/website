...
Building on the idea that [recipes should be flowcharts](/articles/06-flowchart-recipes), this post explores how to convert traditional text-based recipes into clear, visual flowcharts.
Using D3.js, a node network is generated from a recipe. The process steps and ingredients are draggable for flexibility and clarity.
...
Let's start with some examples based on the recipes from [the previous post on the topic](/articles/06-flowchart-recipes). They showcase the current capabilities of the flowchart generator. 
